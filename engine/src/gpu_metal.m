...
struct gpu_context {
    ...
    // Phase G2: Attention and expert kernels
    id<MTLComputePipelineState> attention_forward_ps;
    id<MTLComputePipelineState> expert_mlp_forward_ps;
    id<MTLComputePipelineState> router_forward_ps;
    id<MTLComputePipelineState> topk_softmax_ps;
...
static NSString *load_metal_shader_source(void) {
    // Phase G2: Load ALL .metal files from src/metal/ and concatenate them.
    // This includes ops.metal, attention.metal, mlp.metal, norm.metal, quant.metal.
    
    const char *shader_dir = "src/metal";
    const char *files[] = {"ops.metal", "attention.metal", "mlp.metal", "norm.metal", "quant.metal"};
    int n_files = 5;
    ...
    for (int i = 0; i < n_files; i++) {
        char path[256];
        snprintf(path, sizeof(path), "%s/%s", shader_dir, files[i]);
        FILE *fp = fopen(path, "r");
        if (!fp) continue;
        fseek(fp, 0, SEEK_END);
        long len = ftell(fp);
        fseek(fp, 0, SEEK_SET);
        char *tmp = (char *)realloc(buf, total + (size_t)len + 2);
        if (!tmp) { free(buf); fclose(fp); return @""; }
        buf = tmp;
        fread(buf + total, 1, (size_t)len, fp);
        buf[total + (size_t)len] = '\n';
        total += (size_t)len + 1;
        fclose(fp);
    }
    ...
    // Fall back to compiled-in string (ops.metal only)
    return @R"(
...
gpu_context *gpu_init(void) {
    ...
    @autoreleasepool {
        ...
        // Phase G2: Attention and expert kernels
        ctx->attention_forward_ps = compile_kernel(ctx->device, ctx->library, @"attention_forward", &err);
        ctx->expert_mlp_forward_ps = compile_kernel(ctx->device, ctx->library, @"expert_mlp_forward", &err);
        ctx->router_forward_ps    = compile_kernel(ctx->device, ctx->library, @"router_forward", &err);
        ctx->topk_softmax_ps      = compile_kernel(ctx->device, ctx->library, @"topk_softmax", &err);

        // Don't fail if optional kernels aren't found - they're not called on the CPU-fallback path
        // attention_forward, expert_mlp_forward, router_forward, topk_softmax are optional
...
void gpu_destroy(gpu_context *ctx) {
    ...
    @autoreleasepool {
        ...
        ctx->add_ps = nil;
        ctx->softmax_ps = nil;
        ctx->attention_forward_ps = nil;
        ctx->expert_mlp_forward_ps = nil;
        ctx->router_forward_ps = nil;
        ctx->topk_softmax_ps = nil;
...
void gpu_attention(gpu_context *ctx, gpu_buffer *out,
    ...
    // Phase G2: Dispatch Metal attention kernel if available
    if (!ctx || !out || !Q || !K_cache || !V_cache || seq_len <= 0 || head_dim <= 0) return;
    
    if (!ctx->attention_forward_ps) return;  // kernel not compiled
    
    id<MTLBuffer> mQ = Q->buffer;
    id<MTLBuffer> mK = K_cache->buffer;
    id<MTLBuffer> mV = V_cache->buffer;
    id<MTLBuffer> mO = out->buffer;
    ...
    @autoreleasepool {
        id<MTLCommandBuffer> cb = [ctx->queue commandBuffer];
        id<MTLComputeCommandEncoder> enc = [cb computeCommandEncoder];
        [enc setComputePipelineState:ctx->attention_forward_ps];
        [enc setBuffer:mQ offset:0 atIndex:0];
        [enc setBuffer:mK offset:0 atIndex:1];
        [enc setBuffer:mV offset:0 atIndex:2];
        [enc setBuffer:mO offset:0 atIndex:3];
...
void gpu_expert_mlp(gpu_context *ctx, gpu_buffer *out,
    ...
    // Phase G2: Dispatch Metal expert MLP kernel if available
    (void)quant;
    if (!ctx || !out || !x || !gate_w || !up_w || !down_w) return;
    
    if (!ctx->expert_mlp_forward_ps) return;  // kernel not compiled
    
    id<MTLBuffer> mX = x->buffer;
    id<MTLBuffer> mG = gate_w->buffer;
    id<MTLBuffer> mU = up_w->buffer;
    id<MTLBuffer> mD = down_w->buffer;
    id<MTLBuffer> mO = out->buffer;
    
    @autoreleasepool {
        id<MTLCommandBuffer> cb = [ctx->queue commandBuffer];
        id<MTLComputeCommandEncoder> enc = [cb computeCommandEncoder];
        [enc setComputePipelineState:ctx->expert_mlp_forward_ps];
        [enc setBuffer:mX offset:0 atIndex:0];
        [enc setBuffer:mG offset:0 atIndex:1];
        [enc setBuffer:mU offset:0 atIndex:2];
        [enc setBuffer:mD offset:0 atIndex:3];
        [enc setBuffer:mO offset:0 atIndex:4];
        [enc setBytes:&d_model length:sizeof(int) atIndex:5];
        [enc setBytes:&hidden_dim length:sizeof(int) atIndex:6];
...
void gpu_router(gpu_context *ctx, gpu_buffer *out,
    ...
    // Phase G2: Dispatch Metal router kernel if available
    if (!ctx || !out || !x || !router_weight) return;
    
    if (!ctx->router_forward_ps) return;  // kernel not compiled
    
    id<MTLBuffer> mX = x->buffer;
    id<MTLBuffer> mR = router_weight->buffer;
    id<MTLBuffer> mO = out->buffer;
    
    @autoreleasepool {
        id<MTLCommandBuffer> cb = [ctx->queue commandBuffer];
        id<MTLComputeCommandEncoder> enc = [cb computeCommandEncoder];
        [enc setComputePipelineState:ctx->router_forward_ps];
        [enc setBuffer:mX offset:0 atIndex:0];
        [enc setBuffer:mR offset:0 atIndex:1];
        [enc setBuffer:mO offset:0 atIndex:2];
        [enc setBytes:&d_model length:sizeof(int) atIndex:3];
        [enc setBytes:&n_experts length:sizeof(int) atIndex:4];
...
void gpu_topk_softmax(gpu_context *ctx, uint32_t *indices, float *weights,
    ...
    // Phase G2: Read logits from GPU buffer, do topk+softmax on CPU
    // (A proper parallel top-k kernel requires a radix sort — CPU is fine for 256 experts)
    (void)ctx;
    if (!indices || !weights || !logits) return;
    
    float *logit_data = (float *)[logits->buffer contents];
    int n = n_experts < 1024 ? n_experts : 1024;
    int k = top_k < n ? top_k : n;
    
    typedef struct { float s; int i; } si_t;
    si_t scores[1024];
    for (int i = 0; i < n; i++) { scores[i].s = logit_data[i]; scores[i].i = i; }
    for (int i = 0; i < n - 1; i++)
        for (int j = i + 1; j < n; j++)
            if (scores[j].s > scores[i].s) { si_t t = scores[i]; scores[i] = scores[j]; scores[j] = t; }
    
    float max_s = scores[0].s;
    float sum_e = 0;
    for (int i = 0; i < k; i++) { float e = expf(scores[i].s - max_s); weights[i] = e; indices[i] = (uint32_t)scores[i].i; sum_e += e; }
    for (int i = 0; i < k; i++) weights[i] /= sum_e;
}
...
void gpu_print_info(gpu_context *ctx) {
    ...
    if ([ctx->device hasUnifiedMemory])
        printf("Unified:  yes\n");
    printf("Kernels:  %s%s%s%s%s\n",
           ctx->matmul_vec_ps ? "matmul_vec " : "",
           ctx->attention_forward_ps ? "attention " : "",
           ctx->expert_mlp_forward_ps ? "expert_mlp " : "",
           ctx->router_forward_ps ? "router " : "",
           ctx->topk_softmax_ps ? "topk_softmax " : "");
}
